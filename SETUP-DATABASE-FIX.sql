-- ============================================================================
-- PATCHPULSE: DATABASE SETUP (Fixed for existing tables)
...
ALTER TABLE games ADD COLUMN IF NOT EXISTS platforms TEXT[] DEFAULT ARRAY[]::TEXT[];

-- Add missing columns to patch_notes
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS version TEXT;
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS summary_tldr TEXT;
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS key_changes JSONB DEFAULT '[]'::jsonb;
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS tags TEXT[] DEFAULT ARRAY[]::TEXT[];
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS impact_score INTEGER DEFAULT 50;
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS source_url TEXT;
ALTER TABLE patch_notes ADD COLUMN IF NOT EXISTS raw_content TEXT;

-- Add missing columns to news_items
ALTER TABLE news_items ADD COLUMN IF NOT EXISTS why_it_matters TEXT;
ALTER TABLE news_items ADD COLUMN IF NOT EXISTS topics TEXT[] DEFAULT ARRAY[]::TEXT[];
ALTER TABLE news_items ADD COLUMN IF NOT EXISTS is_rumor BOOLEAN DEFAULT false;
...
DROP POLICY IF EXISTS "Patch notes are viewable by everyone" ON patch_notes;
DROP POLICY IF EXISTS "Service role can manage patch notes" ON patch_notes;
DROP POLICY IF EXISTS "News is viewable by everyone" ON news_items;
DROP POLICY IF EXISTS "Service role can manage news" ON news_items;
...
DROP POLICY IF EXISTS "Users can view own followed games" ON user_followed_games;
DROP POLICY IF EXISTS "Users can manage own followed games" ON user_followed_games;
...
-- ============================================================================
-- DONE! Database is ready for real-time notifications.
-- ============================================================================
