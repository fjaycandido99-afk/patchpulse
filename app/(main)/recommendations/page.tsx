...
const MOMENTUM_CONFIG = {
  ...
  cooling: { icon: TrendingDown, color: 'text-zinc-400', bg: 'bg-zinc-500/20', label: 'Cooling' },
}
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            AI-powered game recommendations based on your backlog
          </p>
        </div>

        <div className="flex flex-col items-center justify-center py-20 px-6 rounded-xl border border-border bg-card">
          <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center mb-4">
            <Sparkles className="w-10 h-10 text-primary" />
          </div>
          <h2 className="text-xl font-semibold mb-2">Pro Feature</h2>
          <p className="text-muted-foreground text-center max-w-md mb-6">
            Get personalized AI recommendations based on your mood, available time, and gaming history.
          </p>
          <Link
            href="/pro"
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Recommendations</h1>
          <p className="mt-2 text-muted-foreground">
            AI-powered suggestions from your backlog + trending discoveries
          </p>
        </div>
      </div>
      ...
      {loading && !result && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-muted-foreground">Analyzing your backlog...</p>
        </div>
      )}
      ...
      {result && (
        <div className="space-y-4">
          ...
          {visibleRecommendations.length === 0 && dismissedIds.size === 0 && !loading && (
            <div className="text-center py-12 text-muted-foreground rounded-xl border border-border bg-card">
              <Gamepad2 className="w-12 h-12 mx-auto mb-4 opacity-50" />
              <p>No recommendations available.</p>
              <p className="text-sm mt-1">Add games to your backlog to get personalized suggestions.</p>
            </div>
          )}
                ...
                <div key={rec.game_id} className="relative group">
                  ...
                  >
                    <div className="flex gap-4 p-4">
                      {/* Game Cover */}
                      <div className={`relative flex-shrink-0 w-24 h-32 rounded-lg overflow-hidden bg-zinc-800 ${
                        isDiscovery
                        ...
                        {/* Rank badge */}
                        <div className="absolute -top-1 -left-1 w-8 h-8 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-bold text-sm shadow-lg">
                          {i + 1}
                        </div>
                        {/* Momentum indicator */}
                        {isHot && !rec.is_discovery && (
                          <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-green-500 flex items-center justify-center shadow-lg animate-pulse">
                            <TrendingUp className="w-4 h-4 text-white" />
                          </div>
                        )}
                        {/* Discovery badge */}
                        {rec.is_discovery && (
                          <div className="absolute -bottom-1 -right-1 px-2 py-0.5 rounded-full bg-amber-500 text-xs font-bold text-black shadow-lg">
                            NEW
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0 pr-8">
                        ...
                        <h3 className="font-semibold text-lg truncate">{rec.game_name}</h3>
                        ...
                        <p className="text-sm text-muted-foreground mt-2 line-clamp-2">{rec.reason}</p>
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 p-2 rounded-full bg-zinc-800/80 text-zinc-400 opacity-0 group-hover:opacity-100 hover:bg-zinc-700 hover:text-zinc-200 transition-all disabled:opacity-50"
